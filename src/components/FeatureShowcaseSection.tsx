...
export const FeatureShowcaseSection = ({ onOpenDemo }: FeatureShowcaseSectionProps) => {
  ...
  // Mobile specific image refs
  const oxalaMobileRef = useRef<HTMLImageElement>(null)
  const yemanjanMobileRef = useRef<HTMLImageElement>(null)

  // GSAP ScrollTrigger: Extended Smooth Scrub Slide-In for Oxalá and Yemanjá (Desktop + Mobile)
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const oxalaMobile = oxalaMobileRef.current
      const yemanMobile = yemanjanMobileRef.current

      // Desktop Animations
      if (container && oxala && yemanja) {
        gsap.fromTo(
          oxala,
          { x: '-75%', opacity: 0 },
          {
      ...
      // Mobile Animations (slide in from screen outer edges at section bottom)
      if (container && oxalaMobile && yemanMobile) {
        gsap.fromTo(
          oxalaMobile,
          { x: '-90%', opacity: 0 },
          {
  ...
  return (
    <section
      ref={containerRef}
      className="light-bg-section relative w-full min-h-screen bg-[#F7F1E6] text-[#262626] py-16 sm:py-20 md:py-24 px-4 sm:px-8 md:px-16 overflow-hidden flex flex-col justify-center select-none"
    >
      ...
      {/* Mobile Images (Oxalá & Yemanjá in front of cards z-30, overlapping bottom edge of Card 4) */}
      <div className="flex lg:hidden absolute left-[-14vw] xs:left-[-10vw] sm:left-[-5vw] bottom-[-38px] xs:bottom-[-42px] pointer-events-none z-30 items-end justify-start max-w-[55vw]">
        <img
          ref={oxalaMobileRef}
          src="/img/oxala.png"
          alt="Oxalá"
          className="h-[420px] xs:h-[480px] sm:h-[540px] w-auto object-contain object-bottom filter drop-shadow-2xl scale-105"
        />
      </div>

      <div className="flex lg:hidden absolute right-[-14vw] xs:right-[-10vw] sm:right-[-5vw] bottom-[-38px] xs:bottom-[-42px] pointer-events-none z-30 items-end justify-end max-w-[55vw]">
        <img
          ref={yemanjanMobileRef}
          src="/img/yeman.png"
          alt="Yemanjá"
          className="h-[420px] xs:h-[480px] sm:h-[540px] w-auto object-contain object-bottom filter drop-shadow-2xl scale-105"
        />
      ...
      <div className="relative z-20 w-full max-w-[1280px] mx-auto flex-1 flex flex-col justify-center items-center py-6 sm:py-12 md:py-16 pb-52 xs:pb-60 sm:pb-64 lg:pb-16">
        <div className="max-w-[620px] w-full mx-auto space-y-5 sm:space-y-7 text-center">
          
          <div className="text-xs md:text-sm font-sans tracking-[0.22em] sm:tracking-[0.25em] uppercase font-semibold text-[#761D19]">
            — TECNOLOGIA LITÚRGICA —
          </div>

          <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl leading-[1.08] sm:leading-[1.06] tracking-tight text-[#262626]">
            Projetado para as Necessidades do <span className="font-serif italic font-normal text-[#990000]">Seu ilê</span>.
          </h2>

          <p className="font-sans text-sm md:text-base font-light text-[#262626]/75 max-w-lg mx-auto leading-relaxed px-1 sm:px-0">
            Passe o cursor sobre os módulos abaixo para visualizar como simplificamos a gestão sagrada e comunitária do seu terreiro.
          </p>

          {/* Cards Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4 pt-2 text-left">
            {features.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  onClick={onOpenDemo}
                  className="group relative bg-[#F7F1E6]/90 backdrop-blur-sm border border-white/80 hover:border-white p-4 xs:p-5 sm:p-6 rounded-2xl md:rounded-3xl shadow-[0_8px_25px_-6px_rgba(0,0,0,0.06),0_2px_8px_-2px_rgba(118,29,25,0.04)] hover:shadow-[0_16px_35px_-8px_rgba(118,29,25,0.12),0_4px_16px_-4px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                >
                  {/* Vertical Portrait Preview Tooltip */}
                  <div
